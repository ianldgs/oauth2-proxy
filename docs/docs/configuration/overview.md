...
| `--skip-auth-route` | string \| list | bypass authentication for requests that match the method & path. Format: method=path_regex OR path_regex alone for all methods. Supports feature rich regexes with [regexp2](https://github.com/dlclark/regexp2).  | |
